...
func TestDenialOfExistenceNSEC_BitMap(t *testing.T) {
	...
	rrset := []dns.RR{
		newRR("test.example.com. 3600 IN NSEC \000.test.example.com. A RRSIG NSEC"),
	}
	...
	nsec := newDenialOfExistenceNSEC(context.Background(), zoneName, set)
...
func TestDenialOfExistenceNSEC_NXDOMAIN(t *testing.T) {
	...
	rrset1 := []dns.RR{
		newRR("example.com. 3600 IN NSEC d.example.com. SOA RRSIG NSEC"),
	}
	rrset2 := []dns.RR{
		newRR("s.example.com. 3600 IN NSEC u.example.com. A RRSIG NSEC"),
	}
	...
	set, err := authenticate(zoneName, slices.Concat(rrset1, rrset2), []*dns.DNSKEY{key.key}, answerSection)
	if err != nil || !set.Valid() {
		panic("cannot setup test")
	}
